...
/** Tipi/asset per cui Stooq non ha ticker affidabili — evita 404 rumorosi. */
const STOOQ_SKIP_TYPES = new Set(['index', 'volatility', 'macro']);

export function stooqSupported(symbol, type) {
  if (STOOQ_SKIP_TYPES.has(type)) return false;
  const id = String(symbol).trim();
  if ((type === 'rates' || type === 'sentiment' || type === 'volatility') && id.startsWith('^')) {
    return false;
  }
  if (type === 'macro' && (id.includes('.') || id.startsWith('^') || id.startsWith('DX-'))) {
    return false;
  }
  return true;
}
...
export function toStooqSymbol(symbol, type) {
  if (!stooqSupported(symbol, type)) return null;
  ...
  if (isEtfType(type) || type === 'rates' || type === 'sentiment') {
...
async function requestLatestCsv(stooqSymbols) {
  const tickers = (Array.isArray(stooqSymbols) ? stooqSymbols : [stooqSymbols]).filter(Boolean);
