...
export async function ensureSchema() {
  ...
  `;
  await db`CREATE INDEX IF NOT EXISTS idx_telegram_link_codes_code ON telegram_link_codes(code)`;
  await db`CREATE INDEX IF NOT EXISTS idx_telegram_link_codes_user ON telegram_link_codes(user_id)`;

  await db`ALTER TABLE portfolio_assets ADD COLUMN IF NOT EXISTS last_value NUMERIC(20, 4)`;
  await db`ALTER TABLE portfolio_assets ADD COLUMN IF NOT EXISTS last_pl_percent NUMERIC(12, 6)`;
  await db`ALTER TABLE portfolio_assets ADD COLUMN IF NOT EXISTS value_updated_at TIMESTAMPTZ`;
  ...
  await db`CREATE INDEX IF NOT EXISTS idx_portfolio_variations_user_time ON portfolio_variations(user_id, created_at DESC)`;
  await db`CREATE INDEX IF NOT EXISTS idx_portfolio_variations_user_importance ON portfolio_variations(user_id, importance DESC)`;
  await db`CREATE INDEX IF NOT EXISTS idx_notification_events_user_time ON notification_events(user_id, created_at DESC)`;
  await db`CREATE INDEX IF NOT EXISTS idx_notification_events_user_importance ON notification_events(user_id, importance DESC)`;
