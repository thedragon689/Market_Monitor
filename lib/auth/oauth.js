/**
 * OAuth social login — Google (ID token), GitHub (code o access token), Apple (ID token).
 */
import bcrypt from 'bcryptjs';
import { getDb } from '../db.js';
import { issueTokenPair } from '../portfolio/auth.js';
import { auditLog } from '../auditLog.js';
import { verifyAppleIdToken, isAppleOAuthConfigured } from './apple.js';
...
async function findOrCreateOAuthUser(provider, profile) {
  ...
  if (!profile.email) {
    throw new Error(
      'Email non disponibile dal provider. Per Apple riprova revocando l\'app in Impostazioni Apple ID, oppure usa email/password.'
    );
  }
 ...
 * @param {string} token — ID token (Google/Apple) o access token GitHub
...
export async function loginWithOAuth(provider, token, opts = {}) {
  ...
  } else if (p === 'apple') profile = await verifyAppleIdToken(token);
...
export function isOAuthConfigured(provider) {
  const p = String(provider).toLowerCase();
  if (p === 'google') return Boolean(process.env.GOOGLE_CLIENT_ID?.trim());
  if (p === 'github') {
    return Boolean(
      process.env.GITHUB_CLIENT_ID?.trim() && process.env.GITHUB_CLIENT_SECRET?.trim()
    );
  }
  if (p === 'apple') return isAppleOAuthConfigured();
  return false;
}
...
export function getOAuthPublicConfig() {
  return {
    ...
    apple: process.env.APPLE_CLIENT_ID?.trim() || process.env.VITE_APPLE_CLIENT_ID?.trim() || '',
  };
}
