...
async function exchangeGoogleCode(code, redirectUri) {
  ...
  if (!clientId || !clientSecret) {
    throw new Error('GOOGLE_CLIENT_ID/SECRET non configurati per il flusso authorization code');
  }
  const uri = redirectUri || process.env.APP_BASE_URL?.trim() || '';
  ...
  if (!res.ok || data.error) {
    throw new Error(data.error_description || data.error || 'Scambio code Google fallito');
  }
  if (!data.id_token) throw new Error('ID token Google mancante');
...
async function findOrCreateOAuthUser(provider, profile) {
  ...
  if (existing[0]) {
    const oauthLinked = await db`
      ...
      LIMIT 1
    `;
    if (!existing[0].email_verified && !oauthLinked.length && !profile.emailVerified) {
      throw new Error(
        'Account email già registrato con password. Accedi con password o verifica l\'email prima di collegare OAuth.'
      );
    }
    userId = existing[0].id;
    if (profile.emailVerified && !existing[0].email_verified) {
...
export async function loginWithOAuth(provider, token, opts = {}) {
  ...
  if (p === 'google') {
    profile = opts.code
      ? await exchangeGoogleCode(opts.code, opts.redirectUri)
...
export function isOAuthConfigured(provider) {
  ...
  const p = String(provider).toLowerCase();
  if (p === 'google') {
    return Boolean(pub.google && process.env.GOOGLE_CLIENT_SECRET?.trim());
  }
  if (p === 'github') {
