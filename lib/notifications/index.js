...
export async function dispatchPortfolioAlert(user, messages) {
  ...
  if (user.whatsapp_number && whatsappText) {
    try {
      results.whatsapp = await sendWhatsApp(user.whatsapp_number, whatsappText);
    } catch (err) {
...
function trendFromSpark(points) {
...
export function buildPushPayload(args) {
  ...
  return {
    ...
    icon: '/app-icon-light-192.png',
    badge: '/app-icon.svg',
    tag: `portfolio-alert-${symbol}-${args.kind}`,
    requireInteraction: true,
    renotify: true,
    silent: false,
    vibrate: [200, 100, 200],
