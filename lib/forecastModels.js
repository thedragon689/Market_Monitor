...
/**
 * Previsione log-return: media r̄ dei rendimenti ln(Pₜ/Pₜ₋₁) su finestra N (o serie intera).
 * Pₜ₊ₖ = Pₜ · e^(k·r̄)
 */
export function logReturnForecast(prices, daysAhead = 1, windowSize = null) {
  const all = (prices || []).map(Number).filter((p) => Number.isFinite(p) && p > 0);
  ...
  for (let i = 1; i <= daysAhead; i++) {
    price *= Math.exp(avgR);
    forecasts.push({ dayOffset: i, dayIndex: all.length + i, price });
  }
