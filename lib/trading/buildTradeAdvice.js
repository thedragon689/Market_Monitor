...
function technicalSignals(analysis, lastPrice) {
  ...
  const cciVal = ind.cci20;
  if (cciVal != null) {
    if (cciVal >= 100) {
      pushSignal(signals, {
        id: 'cci-overbought',
        category: 'technical',
        label: 'CCI ipercomprato',
        direction: 'bear',
        weight: -1,
        detail: `CCI(20) = ${cciVal.toFixed(1)} — estensione sopra la media.`,
      });
    } else if (cciVal <= -100) {
      pushSignal(signals, {
        id: 'cci-oversold',
        category: 'technical',
        label: 'CCI ipervenduto',
        direction: 'bull',
        weight: 1,
        detail: `CCI(20) = ${cciVal.toFixed(1)} — possibile rimbalzo.`,
      });
    }
  }

  const wr = ind.williamsR14;
  if (wr != null) {
    if (wr >= -20) {
      pushSignal(signals, {
        id: 'williams-overbought',
        category: 'technical',
        label: 'Williams %R ipercomprato',
        direction: 'bear',
        weight: -1,
        detail: `Williams %R = ${wr.toFixed(1)}.`,
      });
    } else if (wr <= -80) {
      pushSignal(signals, {
        id: 'williams-oversold',
        category: 'technical',
        label: 'Williams %R ipervenduto',
        direction: 'bull',
        weight: 1,
        detail: `Williams %R = ${wr.toFixed(1)}.`,
      });
  ...
  const mom = ind.momentum14;
  if (mom != null && Math.abs(mom) >= 2) {
    pushSignal(signals, {
      id: 'momentum-14',
      category: 'technical',
      label: mom > 0 ? 'Momentum 14 positivo' : 'Momentum 14 negativo',
      direction: mom > 0 ? 'bull' : 'bear',
      weight: mom > 5 ? 1.5 : mom < -5 ? -1.5 : mom > 0 ? 1 : -1,
      detail: `Variazione su 14 periodi: ${mom >= 0 ? '+' : ''}${mom.toFixed(2)}%.`,
    });
  }
...
function intelligenceSignals(intelligence) {
  ...
  const strong = pairs.find((p) => Math.abs(p.correlation ?? p.rho) >= 0.65);
  if (strong) {
    const rho = strong.correlation ?? strong.rho;
    pushSignal(signals, {
      id: 'correlation',
      ...
      direction: 'neutral',
      weight: 0,
      detail: `ρ = ${rho?.toFixed(2)} — considera diversificazione.`,
    });
  }
...
function forecastSignals(forecast, lastPrice) {
  ...
  const target =
    (typeof combined?.combined === 'number' ? combined.combined : null) ??
    (typeof forecast.combinedForecast === 'number' ? forecast.combinedForecast : null) ??
    forecast.adjustedForecast ??
    combined?.adjustedPrice ??
    forecast.hybrid?.combined ??
    forecast.hybrid?.forecasts?.at(-1)?.price ??
    scenarios?.hybrid?.at(-1)?.price ??
    forecast.forecast?.methods?.linearRegression?.forecasts?.at(-1)?.price ??
