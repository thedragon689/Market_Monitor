...
export async function registerUser(email, password, options = {}) {
  ...
  const phone = normalizePhoneNumber(options.phoneNumber);
  const prefs = options.notificationPrefs ?? {};
  const pushAlerts =
    prefs.pushAlerts !== false && prefs.pushAlerts !== 'false' && prefs.pushAlerts !== 0;
  const notifyGain = prefs.notifyGain !== false && prefs.notifyGain !== 'false';
  const notifyLoss = prefs.notifyLoss !== false && prefs.notifyLoss !== 'false';
  const notifyForecast = prefs.notifyForecast !== false && prefs.notifyForecast !== 'false';
  const notifyAdvice = prefs.notifyAdvice !== false && prefs.notifyAdvice !== 'false';
  ...
  try {
    rows = await db`
      INSERT INTO users (
        ...
        push_alerts_enabled, notify_gain, notify_loss, notify_forecast, notify_advice
      )
      VALUES (
        ${normalized}, ${hash}, FALSE,
        ${phone}, ${phone},
        ${pushAlerts}, ${notifyGain}, ${notifyLoss}, ${notifyForecast}, ${notifyAdvice}
      )
      RETURNING id, email, created_at, phone_number,
        push_alerts_enabled, notify_gain, notify_loss, notify_forecast, notify_advice
    `;
  } catch (err) {
  ...
  await auditLog(user.id, 'auth.register', { phone: phone.slice(0, 6) + '…' });

  try {
    const { createNotificationEvent } = await import('./events.js');
    await createNotificationEvent({
      userId: user.id,
      kind: 'register',
      title: 'Account creato',
      body: `Registrazione completata il ${new Date(user.created_at).toLocaleString('it-IT')}`,
      importance: 0,
      direction: 'neutral',
      payload: { email: user.email, createdAt: user.created_at },
    });
  } catch (err) {
    console.warn('[register] notification event:', err.message);
  }
  ...
  return {
    user: {
      ...
      notificationPrefs: {
        pushAlerts: user.push_alerts_enabled,
        notifyGain: user.notify_gain,
        notifyLoss: user.notify_loss,
        notifyForecast: user.notify_forecast,
        notifyAdvice: user.notify_advice,
      },
    },
    ...tokens,
  };
...
export async function getUserProfile(userId) {
  ...
  const rows = await db`
    SELECT id, email, email_verified, phone_number, auth0_id,
      push_alerts_enabled, notify_gain, notify_loss, notify_forecast, notify_advice
  ...
  return {
    ...
    phoneNumber: user.phone_number,
    auth0: Boolean(user.auth0_id),
    notificationPrefs: {
      pushAlerts: user.push_alerts_enabled !== false,
      notifyGain: user.notify_gain !== false,
      notifyLoss: user.notify_loss !== false,
      notifyForecast: user.notify_forecast !== false,
      notifyAdvice: user.notify_advice !== false,
    },
  };
}
