...
import { loginUser, registerUser, requireAuth, refreshAccessToken, revokeRefreshTokens } from './auth.js';
import {
  clearRefreshCookie,
  readRefreshCookie,
  sendTokenResponse,
...
export function mountPortfolioRoutes(app, deps) {
  ...
  app.post('/api/auth/totp/setup', withDb, requireAuth, async (req, res) => {
    try {
      const db = getDb();
      const secret = generateTotpSecret();
      const users = await db`SELECT email FROM users WHERE id = ${req.userId} LIMIT 1`;
      const email = users[0]?.email || 'user';
      await db`UPDATE users SET totp_secret = ${secret}, totp_enabled = FALSE WHERE id = ${req.userId}`;
      res.json({ secret, uri: totpUri({ secret, email }) });
    } catch (err) {
      res.status(400).json({ error: err.message });
    }
  });

  app.post('/api/auth/totp/enable', withDb, requireAuth, async (req, res) => {
    try {
      const { code } = req.body ?? {};
      const db = getDb();
      const rows = await db`SELECT totp_secret FROM users WHERE id = ${req.userId} LIMIT 1`;
      const secret = rows[0]?.totp_secret;
      if (!secret || !verifyTotp(secret, code)) {
        return res.status(400).json({ error: 'Codice TOTP non valido' });
      }
      await db`UPDATE users SET totp_enabled = TRUE WHERE id = ${req.userId}`;
      res.json({ ok: true });
    } catch (err) {
      res.status(400).json({ error: err.message });
    }
  ...
  app.post('/api/auth/totp/disable', withDb, requireAuth, async (req, res) => {
    try {
      ...
      const pwdOk = password ? await bcrypt.compare(password, user.password_hash) : false;
      const totpOk = code ? verifyTotp(user.totp_secret, code) : false;
      if (!pwdOk && !totpOk) {
        return res.status(401).json({ error: 'Password o codice TOTP richiesti per disabilitare 2FA' });
      }
      await db`UPDATE users SET totp_enabled = FALSE, totp_secret = NULL WHERE id = ${req.userId}`;
      res.json({ ok: true });
    } catch (err) {
      res.status(400).json({ error: err.message });
    }
