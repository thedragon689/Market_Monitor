...
} from '../notifications/telegramBot.js';
import {
  handleWhatsAppWebhook,
  handleWhatsAppWebhookVerify,
  createWhatsAppLinkForUser,
  getWhatsAppBusinessNumber,
  verifyWhatsAppInternalRequest,
} from '../notifications/whatsappWebhook.js';
import { isWhatsAppCloudConfigured } from '../notifications/sendWhatsAppCloud.js';
...
export function mountPortfolioRoutes(app, deps) {
  ...
  app.get('/api/portfolio/getAssets', withDb, requireAuth, async (req, res) => {
    try {
      const sort = String(req.query.sort || 'symbol_asc');
      const assets = await getAssets(req.userId, deps, { sort });
      res.json({ assets });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  ...
  /** Variazioni valore ordinate per importanza o tempo. */
  app.get('/api/portfolio/variations', withDb, requireAuth, async (req, res) => {
    try {
      const sort = String(req.query.sort || 'importance_desc');
      const direction = req.query.direction ? String(req.query.direction) : null;
      const limit = Number(req.query.limit) || 50;
      const variations = await getVariations(req.userId, { sort, direction, limit });
      res.json({ sort, direction, variations });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  });

  /** Timeline progressiva: registrazione → inserimenti → transazioni → variazioni. */
  app.get('/api/portfolio/timeline', withDb, requireAuth, async (req, res) => {
    try {
      const order = String(req.query.order || 'asc').toLowerCase() === 'desc' ? 'desc' : 'asc';
      const limit = Number(req.query.limit) || 100;
      const timeline = await getPortfolioTimeline(req.userId, { order, limit });
  ...
  app.post('/api/notifications/registerTelegram', withDb, async (req, res, next) => {
    const { chatId, telegram_chat_id, userId } = req.body ?? {};
    const resolvedChatId = chatId ?? telegram_chat_id;

    if (userId && resolvedChatId && verifyTelegramInternalRequest(req)) {
      try {
        const result = await registerTelegram(userId, resolvedChatId);
    ...
    return requireAuth(req, res, async () => {
      try {
        if (!resolvedChatId) {
          return res.status(400).json({ error: 'chatId mancante' });
        }
        const result = await registerTelegram(req.userId, resolvedChatId);
        res.json(result);
      } catch (err) {
        res.status(400).json({ error: err.message });
      }
  ...
  /** Deep link t.me/bot?start=<userId> per collegare Telegram dopo registrazione. */
  app.post('/api/notifications/telegram/link', withDb, requireAuth, async (req, res) => {
    try {
      const link = await createTelegramLinkForUser(req.userId);
      res.json(link);
    } catch (err) {
      res.status(400).json({ error: err.message });
    }
  ...
  /** Webhook chiamato da Telegram (nessun JWT — opzionale TELEGRAM_WEBHOOK_SECRET). */
  app.post('/api/telegram/webhook', withDb, async (req, res) => {
    try {
      const { status } = await handleTelegramWebhook(req);
  ...
  app.post('/api/notifications/registerWhatsApp', withDb, async (req, res) => {
    const { phoneNumber, whatsapp_number, phone, userId } = req.body ?? {};
    const resolvedPhone = phoneNumber ?? whatsapp_number ?? phone;

    if (userId && resolvedPhone && verifyWhatsAppInternalRequest(req)) {
      try {
        const result = await registerWhatsApp(userId, resolvedPhone);
        return res.json({ ok: true, ...result });
      } catch (err) {
        return res.status(400).json({ error: err.message });
      }
    ...
    return requireAuth(req, res, async () => {
      try {
        if (!resolvedPhone) {
          return res.status(400).json({ error: 'phone o phoneNumber mancante' });
        }
        const result = await registerWhatsApp(req.userId, resolvedPhone);
        res.json(result);
      } catch (err) {
        res.status(400).json({ error: err.message });
      }
  ...
  /** Deep link wa.me con /start <userId> per collegare WhatsApp automaticamente. */
  app.post('/api/notifications/whatsapp/link', withDb, requireAuth, async (req, res) => {
    try {
      const link = createWhatsAppLinkForUser(req.userId);
      res.json(link);
    } catch (err) {
      res.status(400).json({ error: err.message });
    }
  ...
  /** Webhook Meta WhatsApp — verifica GET + messaggi POST. */
  app.get('/api/whatsapp/webhook', (req, res) => {
    const { status, body } = handleWhatsAppWebhookVerify(req);
    if (body != null) return res.status(status).send(body);
    res.sendStatus(status);
  });

  app.post('/api/whatsapp/webhook', withDb, async (req, res) => {
    try {
      const { status } = await handleWhatsAppWebhook(req);
      res.sendStatus(status);
    } catch (err) {
  ...
  /** Feed notifiche da DB (importanza / cronologia) per il pannello frontend. */
  app.get('/api/notifications/feed', withDb, requireAuth, async (req, res) => {
    try {
      const sort = String(req.query.sort || 'importance_desc');
      ...
      const [items, insights] = await Promise.all([
        getNotificationFeed(req.userId, { sort, limit, unreadOnly }),
        getNotificationInsights(req.userId),
      ]);
      res.json({ sort, unreadOnly, items, insights });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  });

  app.post('/api/notifications/read', withDb, requireAuth, async (req, res) => {
    try {
      const ids = Array.isArray(req.body?.ids) ? req.body.ids : null;
      const result = await markNotificationsRead(req.userId, ids);
      res.json(result);
    } catch (err) {
      res.status(400).json({ error: err.message });
    }
  ...
  app.get('/api/notifications/config', async (_req, res) => {
    const botUsername = await getTelegramBotUsername().catch(() => null);
    res.json({
      ...
      whatsappBusinessNumber: getWhatsAppBusinessNumber(),
