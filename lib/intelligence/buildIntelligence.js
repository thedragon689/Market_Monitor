...
async function loadBenchmark(symbol, type) {
  try {
    const data = await withTimeout(
      loadMarketData(symbol, type),
      Number(process.env.BENCHMARK_LOAD_TIMEOUT_MS) || 8_000,
      `benchmark ${symbol}`
    );
...
export async function buildMarketIntelligence({
  ...
  if (type === 'stock' || type === 'national') {
    benchmark = 'SPY';
    benchSeries = await loadBenchmark('SPY', 'stock');
  } else if (type === 'crypto') {
    benchmark = 'BTC-USD';
    benchSeries = await loadBenchmark('BTC-USD', 'crypto');
  } else if (type === 'commodity') {
    benchmark = 'WTI';
    benchSeries = await loadBenchmark('WTI', 'commodity');
  } else if (type === 'precious') {
    benchmark = 'XAUUSD';
    benchSeries = await loadBenchmark('XAUUSD', 'precious');
  }
  ...
  const rf = rfRaw
    ? {
        ...
        description:
          'Ensemble su rendimenti log, SMA ratio, sentiment, volatilità e geo.',
