...
export async function buildMarketIntelligence({
  ...
  if (type === 'stock' || type === 'national') {
    ...
    try {
      ...
      benchPrices = btc.series.map((p) => p.price);
    } catch {
      /* optional */
    }
  }

  const risk = await buildRiskProfile(prices, { benchPrices });
  const forecast = buildForecastResponse(prices, { windowSize, horizonDays, methods });
  const poly = polynomialForecast(prices, 2, horizonDays);
  const rf = randomForestPredict(prices, {
